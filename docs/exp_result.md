...
> 最后更新：2026-05-20 23:54 UTC+8
> 所有数据从服务器实际文件读取，非记忆推断。

---

## 0. Seed13 test-set 总览（2026-05-20）

本节只汇总已完成、已写入 `eval/` 的服务器 run。主指标为 `legacy_doc2edag`（Doc2EDAG/ProcNet-style fixed-slot 口径）；`unified_strict` 与 `docfee_official` 作为辅助指标族单独报告，不与主指标混算。

### 0.1 Test 主结果

| Dataset | split | seed | 主设置 | Run root | Legacy-FS F1 | P | R | multi_F1 | single_F1 | Unified F1 | DocFEE F1 |
|---|---|---:|---|---|---:|---:|---:|---:|---:|---:|---:|
| ChFinAnn-Doc2EDAG | test | 13 | vLLM BF16 merged, k=1 greedy | `runs/sarge_chfinann_test_seed13_vllm_bf16_k1_20260520T141531Z/sarge_infer_ChFinAnn-Doc2EDAG_test_20260520T142815Z/` | **0.8547** | 0.8407 | 0.8691 | 0.8082 | 0.8978 | 0.8689 | 0.8604 |
| DuEE-Fin-dev500 | test | 13 | HF 4-bit NF4 + LoRA, k=1 greedy, no-LRD | `runs/sarge_infer_DuEE-Fin-dev500_test_seed13_safe_anchor_source_f56a0d3_20260520T003122Z/sarge_infer_DuEE-Fin-dev500_test_20260520T003122Z/` | **0.7796** | 0.7664 | 0.7933 | 0.7751 | 0.7927 | 0.7888 | 0.7771 |
| DuEE-Fin-dev500 | test | 13 | HF 4-bit NF4 + LoRA, k=1 greedy, safe-anchor LRD | `runs/sarge_postlrd_DuEE-Fin-dev500_test_seed13_safe_anchor_tau0.90_20260520T125328Z/` | **0.7800** | 0.7671 | 0.7933 | 0.7751 | 0.7937 | 0.7892 | 0.7775 |

DuEE-Fin test 上 safe-anchor LRD 的增益很小：Legacy-FS F1 从 `0.7796` 到 `0.7800`，Unified F1 从 `0.7888` 到 `0.7892`。因此当前论文主表仍建议使用 no-LRD 规则路径，LRD 作为诊断/附录结果报告。

### 0.2 Seed13 test 消融

| Dataset | split | seed | Factor | Fixed config | Run root | Legacy-FS F1 | P | R | multi_F1 | single_F1 | 结论 |
|---|---|---:|---|---|---|---:|---:|---:|---:|---:|---|
| DuEE-Fin-dev500 | test | 13 | backend | SFT, k=1 greedy | `runs/sarge_vllm_DuEE-Fin-dev500_test_seed13_bf16_k1_diag_20260520T125819Z/sarge_infer_DuEE-Fin-dev500_test_20260520T130447Z/` | 0.7354 | 0.7402 | 0.7305 | 0.7203 | 0.7722 | vLLM BF16 比 HF 4-bit 主路径低 4.42pp |
| DuEE-Fin-dev500 | test | 13 | no-SFT | vLLM BF16, k=1 greedy | `runs/sarge_vllm_DuEE-Fin-dev500_test_seed13_noSFT_k1_ablation_20260520T145054Z/sarge_infer_DuEE-Fin-dev500_test_20260520T145454Z/` | 0.1129 | 0.3722 | 0.0665 | 0.0936 | 0.1378 | SFT 是主要增益来源 |
| DuEE-Fin-dev500 | test | 13 | decoding | vLLM BF16, SFT | `runs/sarge_vllm_DuEE-Fin-dev500_test_seed13_bf16_k4_T0.7_decoding_ablation_20260520T145457Z/sarge_infer_DuEE-Fin-dev500_test_20260520T150126Z/` | 0.7313 | 0.6922 | 0.7751 | 0.7249 | 0.7583 | k=4 T=0.7 未优于 vLLM k=1 |
| ChFinAnn-Doc2EDAG | test | 13 | no-SFT | vLLM BF16, k=1 greedy | `runs/sarge_vllm_ChFinAnn-Doc2EDAG_test_seed13_noSFT_k1_ablation_20260520T150132Z/sarge_infer_ChFinAnn-Doc2EDAG_test_20260520T150959Z/` | 0.2482 | 0.6119 | 0.1556 | 0.0675 | 0.3893 | SFT 是主要增益来源 |
| ChFinAnn-Doc2EDAG | test | 13 | decoding | vLLM BF16, SFT | `runs/sarge_vllm_ChFinAnn-Doc2EDAG_test_seed13_bf16_k4_T0.7_decoding_ablation_20260520T151014Z/sarge_infer_ChFinAnn-Doc2EDAG_test_20260520T153145Z/` | 0.8421 | 0.7954 | 0.8945 | 0.8071 | 0.8752 | k=4 T=0.7 低于 k=1 主路径 1.26pp |

### 0.3 输出鲁棒性与 exact-record

所有已完成的 seed13 test run 均为 `schema_valid_rate=1.0`，`parse_failure_count=0`，`invalid_event_type_count=0`，`invalid_role_count=0`。Unified-Strict exact-record 诊断如下：

| Run | record_exact_match_count | validated_record_count | exact-record F1 |
|---|---:|---:|---:|
| ChFinAnn test main vLLM k=1 | 2689 | 9637 | 0.5580 |
| ChFinAnn test vLLM k=4 T=0.7 | 2729 | 9933 | 0.5495 |
| ChFinAnn test no-SFT vLLM | 323 | 6330 | 0.1021 |
| DuEE-Fin test main HF no-LRD | 662 | 3090 | 0.4285 |
| DuEE-Fin test safe-anchor LRD | 662 | 3088 | 0.4288 |
| DuEE-Fin test vLLM k=1 | 561 | 2986 | 0.3758 |
| DuEE-Fin test vLLM k=4 T=0.7 | 550 | 3170 | 0.3470 |
| DuEE-Fin test no-SFT vLLM | 3 | 2333 | 0.0026 |

### 0.4 不纳入结果与仍在运行

- SACD/CACD 已废弃，不再作为 seed13 实验项；已停止的 `sarge_vllm_ChFinAnn-Doc2EDAG_dev_seed13_sacd_lax_ablation_20260520T153208Z` 不纳入结果。
- ChFinAnn LRD test 未完成：旧 checkpoint `runs/lrd/chfinann_seed13/checkpoints/lrd_planner.pt` 缺少当前 runtime 需要的 `encoder_role_embed`，不能直接用于评测；若要报告 ChFinAnn LRD，需重新生成 train candidates / preencoded cache / checkpoint。
- 正在运行：`logs/seed13_hf_remaining_queue_20260520T154207Z.log`，PID `2349084`。当前队列包含 `DuEE-Fin test HF no-SFT k=1` 与 `ChFinAnn test HF 4-bit NF4 + LoRA k=1 backend cross-check`，预计 2026-05-21 16:40-18:40 UTC+8 完成。
...
**W8 判定**：safe-anchor LRD seed13 dev 复验通过候选 gate：legacy F1(M.) `0.7550` ≥ rule planner `0.6992` + 3pp，exact-record F1 `0.4181` ≥ `0.40`。W9 test 已完成，见 §7.2 与 §7.3；seed 17/42 多种子尚未执行。
...
对比当前 SARGE：dev seed13 no-LRD exact-record F1 `0.4175`，safe-anchor LRD exact-record F1 `0.4181`；test seed13 no-LRD exact-record F1 `0.4285`，safe-anchor LRD exact-record F1 `0.4288`。因此，在 Unified-Strict exact-record 口径下，SARGE seed13 dev/test 均高于 ProcNet seed42 reference；该对比是同口径诊断，不替代 Legacy-FS 主表。
...
### 7.3 W9 safe-anchor LRD test result

W9 safe-anchor LRD test 使用 §7.2 的 `parsed_candidates.test.jsonl`，LRD checkpoint 为 `runs/lrd/dueefin_train_seed13/checkpoints/lrd_planner.pt`，输出 run root 为 `runs/sarge_postlrd_DuEE-Fin-dev500_test_seed13_safe_anchor_tau0.90_20260520T125328Z/`。

| Track | F1 | P | R | multi_F1 | single_F1 | diagnostics |
|---|---:|---:|---:|---:|---:|---|
| legacy_doc2edag | 0.7800 | 0.7671 | 0.7933 | 0.7751 | 0.7937 | `schema_valid_rate=1.0`, parse/invalid=0 |
| unified_strict | 0.7892 | 0.7774 | 0.8014 | 0.7836 | 0.8035 | `record_exact_match_count=662`, `validated_record_count=3088` |
| docfee_official | 0.7775 | 0.7658 | 0.7896 | 0.7622 | 0.8035 | `schema_valid_rate=1.0`, parse/invalid=0 |

LRD test 增益很小：Legacy-FS F1 `+0.0004`，Unified F1 `+0.0004`，exact-record F1 `0.4285 → 0.4288`。因此当前不把 LRD 作为主表设置，只报告为记录消歧诊断。
...
| W9 | Test set no-LRD + safe-anchor LRD 评测 | ~2-4 GPU-h | 已完成 |
| W10 | Seed13 test ablation 表 | — | 已完成主要 vLLM 消融；HF 长队列仍在运行 |
| W11 | 多种子 seed 17/42 | 高 | 未执行；需后续单独授权 |
