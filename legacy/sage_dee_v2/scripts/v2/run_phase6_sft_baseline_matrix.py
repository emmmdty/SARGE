...
BASELINES = (
    BaselineSpec("S1", "phase6_s1_direct_json", "direct JSON SFT", "direct_json"),
    BaselineSpec("S2", "phase6_s2_schema_only", "schema-only SFT", "schema_only"),
    BaselineSpec("S3", "phase6_s3_role_safe", "role-safe SFT", "role_safe"),
    BaselineSpec(
        "S4",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run SAGE v2 Phase 6 S1-S4 same-backbone SFT baselines.")
    ...
    parser.add_argument("--matrix-summary-name", default="phase6_matrix_summary.json")
...
def _validate_args(args: argparse.Namespace, config: dict[str, Any]) -> None:
    if args.split == "test":
        raise ValueError("Phase 6 rejects test split; test remains blocked")
    if args.split != "dev":
        raise ValueError(f"Phase 6 only permits dev split, got {args.split!r}")
    ...
    if args.stage == "limit50" and not args.allow_limit50 and not args.dry_run:
        raise ValueError("Phase 6 limit50 stage requires --allow-limit50")
    if args.stage == "full-dev" and not args.allow_full_dev:
        raise ValueError("Phase 6 full-dev stage requires --allow-full-dev")
    phase6 = config.get("phase6") or {}
    if int(phase6.get("train_limit") or 0) != 512:
        raise ValueError("Phase 6 train_limit must be frozen at 512")
    if phase6.get("full_train_used") is not False:
        raise ValueError("Phase 6 full train must remain blocked")
    if phase6.get("test_blocked") is not True:
        raise ValueError("Phase 6 config must keep test blocked")
...
def _run_limit50_matrix(
    ...
    *,
    ...
    _write_json(args.out_root / "phase6_profile_freeze_manifest.json", freeze_manifest)
    for spec in run_specs:
        timestamp = _created_slug()
        run_dir = args.out_root / f"phase6_{spec.baseline.baseline_id}_seed{spec.seed}_{timestamp}"
        ...
        config_path = run_dir / "phase6_config.resolved.yaml"
        write_yaml(config_path, run_config)
        _write_json(run_dir / "phase6_profile_freeze_manifest.json", freeze_manifest)
        ...
        adapter_path = train["adapter_dir"] if not args.dry_run else "dry-run-no-adapter"
        limit50_config = _resolved_config(
            config,
            baseline=spec.baseline,
            seed=spec.seed,
            stage="limit50",
        ...
        limit50 = _run_generate(
            args,
            config=limit50_config,
            run_dir=run_dir / "limit50",
            dataset=_dataset(args, config),
            data_root=_data_root(args, config),
            split=args.split,
            limit=int((config.get("phase6") or {}).get("limit50_predict_docs") or 50),
            stage_name="limit50",
        )
        ...
        _write_json(run_dir / "phase6_run_summary.json", summary)
...
def _run_full_dev_matrix(
    ...
    *,
    ...
    for spec in run_specs:
        ...
        if existing is None:
            raise RuntimeError(f"missing completed limit50 run for {spec.baseline.baseline_id} seed {spec.seed}")
        previous = _read_json(existing / "phase6_run_summary.json")
        _require_profile_hash(args, existing)
        _require_limit50_gate(previous)
        adapter_path = str(((previous.get("train") or {}).get("adapter_dir")) or "")
        ...
        _write_json(existing / "phase6_run_summary.json", updated)
...
def _run_train(
    ...
    *,
    ...
    completed = _run_command(
        cmd,
        command_path=train_dir / "phase6_train_command.json",
        stdout_path=train_dir / "phase6_train.stdout.log",
        stderr_path=train_dir / "phase6_train.stderr.log",
    )
    if completed.returncode != 0:
        raise RuntimeError(f"Phase 6 train failed with exit code {completed.returncode}: {completed.stderr}")
...
def _run_generate(
    ...
    *,
    ...
    telemetry = start_qwen_telemetry(
        config,
        run_dir,
        operation=f"phase6_{stage_name}_generate",
    ...
    _write_json(
        run_dir / "generation_manifest.json",
        {
            ...
            "gold_visible": False,
            "phase6_stage": stage_name,
            "generation": _generation_metadata(config),
        },
    )
    _write_json(
        run_dir / "phase6_generate_command.json",
        {
...
def _run_evaluator(
    ...
    *,
    ...
    _write_json(run_dir / "phase6_evaluator_handoff.json", {"handoff": handoff.to_dict(), "result": result})
    return {
...
def _parse_valid_subset(
    ...
    *,
    ...
    shutil.copy2(stage_dir / "run_manifest.json", parse_valid_run / "run_manifest.json")
    subset_root = _write_subset_benchmark(
...
def _run_summary(
    ...
    *,
    ...
    scope = {
        "dataset": _dataset(args, config),
        "split": args.split,
        "train_limit": int((config.get("phase6") or {}).get("train_limit") or 512),
        "limit50": int((config.get("phase6") or {}).get("limit50_predict_docs") or 50),
        "full_dev_used": full_dev is not None,
        "full_train_used": False,
        "test_used": False,
        "parse_only_improvement_claimed": False,
    }
    return {
...
def _require_profile_hash(args: argparse.Namespace, run_dir: Path) -> None:
    manifest = _read_json(run_dir / "phase6_profile_freeze_manifest.json")
    expected = manifest.get("config_sha256")
    actual = _sha256(args.config)
    if expected != actual:
        raise RuntimeError("full dev blocked: Phase 6 config hash differs from frozen limit50 profile")


def _run_specs(config: dict[str, Any], *, args: argparse.Namespace) -> list[RunSpec]:
    seed_matrix = (config.get("phase6") or {}).get("seed_matrix") or {}
    specs = []
    ...
    if not specs:
        raise ValueError("Phase 6 run matrix is empty")
    return specs
...
def _resolved_config(
    ...
    *,
    ...
    data_cfg["max_train_docs"] = int((config.get("phase6") or {}).get("train_limit") or 512)
...
def _select_gpu(config: dict[str, Any], *, force_gpu: str | None = None) -> dict[str, Any]:
    phase6 = config.get("phase6") or {}
    preferred = str(phase6.get("preferred_gpu") or "3")
    auto = bool(phase6.get("auto_select_idle_gpu", True))
    idle_memory_mb = int(phase6.get("idle_memory_mb") or 1024)
    idle_util_pct = int(phase6.get("idle_utilization_pct") or 15)
    fake = os.environ.get("SAGE_DEE_PHASE6_FAKE_NVIDIA_SMI")
...
def _run_manifest(*, config: dict[str, Any], dataset: str, split: str, stage_name: str) -> dict[str, Any]:
    return {
        "run_id": f"phase6_{stage_name}_{_created_slug()}",
        "method_name": "SAGE-DEE-v2-Phase6-SFT",
        ...
        "profile": (config.get("run") or {}).get("profile"),
        "command_train": None,
        "command_infer": join([sys.executable, "scripts/v2/run_phase6_sft_baseline_matrix.py", "--stage", stage_name]),
        "git_commit": _git_commit(),
        "created_at": _created_at(),
        "notes": "Phase 6 SFT baseline generation; test split remains blocked.",
    }
...
def _latest_limit50_run(out_root: Path, spec: RunSpec) -> Path | None:
    pattern = f"phase6_{spec.baseline.baseline_id}_seed{spec.seed}_*"
    candidates = [path for path in out_root.glob(pattern) if (path / "phase6_run_summary.json").is_file()]
    if not candidates:
        return None
